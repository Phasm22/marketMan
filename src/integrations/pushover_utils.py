...
def send_energy_alert(
    signal, confidence, title, reasoning, etfs=None, article_url=None, analysis=None
):
    """
    ...
    signal_indicator = {"Bullish": "↗ BULLISH", "Bearish": "↘ BEARISH", "Neutral": "→ NEUTRAL"}.get(
        signal, "? UNKNOWN"
    )
...
def send_system_alert(service_name, status, details=None):
    ...
    return send_pushover_notification(
        message=message, title=f"System Alert: {service_name}", priority=priority
    )
...
def test_pushover():
    ...
    return send_pushover_notification(
        message="Test notification from marketMan system", title="🧪 Test Alert", priority=0
    )
