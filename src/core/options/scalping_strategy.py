...
class OptionsScalpingStrategy:
    ...
    def manage_positions(self) -> None:
        ...
        for position in self.active_positions[:]:  # Copy list to avoid modification during iteration
            try:
    ...
    def _exit_position(self, position: ScalpingPosition, reason: str) -> None:
        ...
        try:
            ...
            position.pnl = (position.current_price - position.signal.entry_price) * position.quantity
    ...
    def _update_position(self, position: ScalpingPosition) -> None:
        ...
        try:
            ...
            logger.debug(f"Updated {position.signal.symbol}: ${old_price:.2f} -> ${position.current_price:.2f}")
