...
def get_fallback_cover_image(session_analyses):
    ...
    for analysis in session_analyses:
        ...
        if article_link:
            logger.info(
                f"🖼️ Trying cover image from: {source_article.get('title', 'Unknown')[:50]}..."
            )
    ...
    def process_alerts(self):
            ...
            for article in alert["articles"]:
                try:
                    ...
                    analysis = analyze_thematic_etf_news(
                        article["title"], article["snippet"], article["snippet"]
                    )
        ...
        if session_analyses:
            ...
            if consolidated_report:
                ...
                notion_url = self.gmail_poller.log_consolidated_report_to_notion(
                    consolidated_report
                )
                ...
                if high_conviction_signals:
                    logger.info(
                        f"📋 Queueing {len(high_conviction_signals)} high-conviction signals..."
                    )
...
class GmailPoller:
    ...
    def extract_articles_from_html(self, html_body):
        ...
        title_link_pattern = (
            r'<a href="([^"]*)" itemprop="url"[^>]*>\s*<span itemprop="name"[^>]*>(.*?)</span>'
        )
    ...
    def log_to_notion(self, analysis_data):
        ...
        try:
            ...
            data = {
                ...
                "properties": {
                    "Title": {"title": [{"text": {"content": analysis_data.get("title", "")}}]},
                    "Signal": {"select": {"name": analysis_data.get("signal", "Neutral")}},
                    "Confidence": {"number": analysis_data.get("confidence", 0)},
                    "ETFs": {
                        "multi_select": [{"name": etf} for etf in analysis_data.get("etfs", [])]
                    },
                    "Sector": {"select": {"name": analysis_data.get("sector", "Mixed")}},
                    "Reasoning": {"rich_text": [{"text": {"content": full_reasoning}}]},
                    "Action": {"select": {"name": action_recommendation}},
                    "Status": {"select": {"name": "New"}},
                    "Timestamp": {
                        "date": {
                            "start": analysis_data.get("timestamp", datetime.now().isoformat())
                        }
                    },
                    "Link": {"url": analysis_data.get("link", "")},
                    "Search Term": {
            ...
            if confidence >= 7:
                tactical_explanation = generate_tactical_explanation(
                    analysis_data, analysis_data.get("title", "")
                )
                if tactical_explanation:
                    tactical_toggle = {
                        ...
                        "toggle": {
                            "rich_text": [
                                {"type": "text", "text": {"content": "🎯 Tactical Brief"}}
                            ],
                            "children": [
                                {
                                    "object": "block",
                                    ...
                                    "paragraph": {
                                        "rich_text": [
                                            {
                                                "type": "text",
                                                "text": {"content": tactical_explanation},
                                            }
                                        ]
                                    },
                                }
            ...
            if response.status_code == 200:
                ...
                if "object_not_found" in str(error_data):
                    logger.warning(
                        "Notion database not found or not shared with integration. Skipping logging."
                    )
                else:
    ...
    def log_memory_patterns_to_notion(self, patterns):
        ...
        try:
            ...
            for pattern in patterns:
                ...
                pattern_content += (
                    f"**Avg Confidence:** {pattern.get('average_confidence', 0):.1f}/10\n"
                )
                ...
                "properties": {
                    "title": {
                        "title": [
                            {
                                "text": {
                                    "content": f"Memory Patterns - {datetime.now().strftime('%Y-%m-%d')}"
                                }
                            }
                        ]
                    }
    ...
    def log_consolidated_report_to_notion(self, report_data):
        ...
        try:
            ...
            if not etf_list:
                ...
                default_etfs = ["ICLN", "TAN", "QCLN", "PBW", "ITA"][
                    :3
                ]  # CleanTech focus based on recent alerts
            ...
            if strong_buys:
                ...
                for pos in strong_buys[:3]:  # Top 3
                    position_recommendations.append(
                        f"• {pos['ticker']}: Target entry ${pos['entry_price']:.2f} | Conviction {pos['conviction']:.1f}/10 | Volume {pos['volume']:,}"
                    )
                    position_recommendations.append(
                        f"  Strategy: 2-5% position size, stop-loss at -8%, target +15-20%"
                    )
            ...
            if strong_sells:
                ...
                for pos in strong_sells[:2]:  # Top 2
                    position_recommendations.append(
                        f"• {pos['ticker']}: Entry ${pos['entry_price']:.2f} | Conviction {pos['conviction']:.1f}/10"
                    )
                    position_recommendations.append(
                        f"  Strategy: Consider inverse exposure or defensive hedging"
                    )

            if not strong_buys and not strong_sells:
                position_recommendations.append("📊 HOLD STRATEGY:")
                position_recommendations.append(
                    "Market signals show promise but lack strong confirmation."
                )
                position_recommendations.append(
                    "• Monitoring key ETFs for volume breakouts and trend confirmation"
                )
                if watchlist:
                    position_recommendations.append(
                        f"• Watchlist tracking: {', '.join(watchlist[:3])}"
                    )
                position_recommendations.append(
                    "• Waiting for clearer directional signals before major allocation"
                )
            ...
            data = {
                ...
                "properties": {
                    ...
                    "ETFs": {
                        "multi_select": [
                            {"name": etf_name} for etf_name in etf_list[:5]
                        ]  # Limit to 5 to avoid Notion limits
                    },
                    "Link": {
                        "url": first_article_link
                        if first_article_link
                        else "https://example.com"  # Fallback URL
                    },
            ...
            if session_articles:
                ...
                if not cover_image:
                    logger.info(
                        f"⚠️ No cover images found from {len(session_articles)} session articles"
                    )
                        ...
                        "rich_text": [
                            {
                                "type": "text",
                                "text": {
                                    "content": f"⚠️ Risk Assessment - {report_data.get('risk_level', 'Medium')} Risk"
                                },
                            }
                        ],
                        "children": [
                            {
                                ...
                                "paragraph": {
                                    "rich_text": [
                                        {"type": "text", "text": {"content": risk_content.strip()}}
                                    ]
                                },
                            }
                        ],
                            ...
                            "rich_text": [
                                {
                                    "type": "text",
                                    "text": {
                                        "content": f"📰 Source Articles ({len(report_data['session_articles'])})"
                                    },
                                }
                            ],
...
def create_consolidated_signal_report(all_analyses, session_timestamp):
    ...
    dominant_sector = (
        max(primary_sectors.keys(), key=lambda s: primary_sectors[s])
        if primary_sectors
        else "Mixed"
    )
    ...
    report = {
        ...
        if len([a for a in all_analyses if a.get("signal") == "Bullish"]) > len(all_analyses) / 2
        else "Bearish"
        if len([a for a in all_analyses if a.get("signal") == "Bearish"]) > len(all_analyses) / 2
        else "Mixed",
        ...
        "watchlist": [
            etf for etf, data in etf_positions.items() if 0.3 <= abs(data["net_score"]) < 1.0
        ][:10],
        "risk_level": "High"
        if any(a.get("sector") == "Volatility" for a in high_conviction)
        else "Medium",
...
if __name__ == "__main__":
    ...
    if "test" in sys.argv:
        ...
        if analysis_result:
            ...
            focused_etfs, primary_sector = categorize_etfs_by_sector(
                analysis_result.get("affected_etfs", [])
            )
...
def test_consolidated_reporting():
    ...
    mock_analyses = [
        ...
        },
        {
            ...
            "sector": "Energy",
            "market_snapshot": {"XLE": {"price": 84.80, "volume": 27289474}},
    ...
    if consolidated_report:
        ...
        if consolidated_report["strong_buys"]:
            ...
            for pos in consolidated_report["strong_buys"]:
                print(
                    f"   • {pos['ticker']}: ${pos['entry_price']:.2f} (Conviction: {pos['conviction']:.1f}, {pos['signals_count']} signals)"
                )
